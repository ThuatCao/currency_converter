...
class CurrencyBloc extends Bloc<CurrencyEvent, CurrencyState> {
  ...
  CurrencyBloc({required this.db, required this.dioClient})
    ...
    on<InitializedAppCurrencyEvent>((event, emit) async {
      ...
      try {
        ...
        if (hasLocalData) {
          emit(
            CurrencyLoadSuccess(
              currencies: localData,
              lastUpdatedText: "Using Cached Data",
              isFromCache: true,
            ),
          );
        } else {
      ...
      if (event.remainingTicks > 0) {
        emit(
          CurrencyFirstLoadFailure(
            message:
                "Không có kết nối internet. Ứng dụng cần nạp dữ liệu vào lần đầu. Vui lòng kiểm tra lại thiết bị!",
            retryCountdown: event.remainingTicks,
          ),
        );
      } else {
